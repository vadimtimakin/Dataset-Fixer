...
def sorter(current_root, new_root):
    """
    ...
    for root, dirs, files in os.walk(current_root):
        for file in files:
            ...
            if os.path.isfile(os.path.join(root, file)):
                ...
                # This flag checks that the file_type variable
                # is not 'NoneType', which helps avoid TypeError.
                # It sometimes happens that imghdr defines the type of a normal
                # file (for example, a jpg image) as None.
                if file_type:
                    # Create a folder with a name containing the type of files
                    # inside it, if this folder has not been created yet.
                    if not os.path.exists(os.path.join(new_root, file_type)):
